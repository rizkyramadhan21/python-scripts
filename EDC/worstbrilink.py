...
def getLowestAvailability(table, percentThreshold):
	...
	try:
		...
		for i in range (2, numRows-1):
			...
			percentAvail = tdcells[17].getText()
			ukerName = cleanUpNamaUker(str(tdcells[0].getText()))

			#jumlahATM = tdcells[3].getText()

			if (float(percentAvail)<float(percentThreshold)) and ("Others" not in ukerName) and ("Kanwil" not in ukerName) and ("KANINS" not in ukerName.upper()) and ("PT" not in ukerName.upper()):
...
def worstMessage(percentThreshold):
	...
	msgBody = "---------------------------------------\n"
	msgBody = msgBody + "KANCA with LOWEST AVAILABILITY EDC BRILINK\nKANWIL "+RegionName + "\n"
