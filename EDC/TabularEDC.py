...
def getTableContents(table):
	...
	for i in range (numRowsHead, numRows-numRowsFoot):
		...
		if not dText[0].isalpha():
			...
			percentPerUker.append((kodeCabang, namaCabang, NOP, JumEDC, Avail))


	print "RANKING EDC MERCHANT\n"

	merchant = sorted(percentPerUker, key=itemgetter(4, 3), reverse = True)
	...
	brilinks = sorted(percentPerUker, key=itemgetter(4, 3), reverse = True)
...
def putDataXL(offRow, offCol, merchant, brilinks, jenisPersen):
	...
	sheet1.write_merge(offRow, offRow, offCol, offCol+10, 'EDC PRO ' +jenisPersen+' '+ RegionName , xlwt.easyxf(styleTitle))
	shiftDown = 1

	sheet1.row(1).height_mismatch = True
	sheet1.row(1).height = 360
	sheet1.write_merge(offRow+shiftDown, offRow+shiftDown, offCol, offCol+10, 'posisi tanggal ' +time.strftime("%d/%m/%Y-%H:%M") , xlwt.easyxf(styleTitle))
	contentAlignmentHorz = ["center", "right", "center", "center", "left"]
	...
	def makeHeader(xRow, yCol, jenisEDC, jenisPersen):
		...
		sheet1.write_merge(xRow+2*shiftDown, xRow+2*shiftDown, yCol, yCol+4, jenisEDC + ' '+jenisPersen, xlwt.easyxf(styler('sky_blue_10', 240)))
	
		arrJudul = ["CODE", "BRANCH", "NOP", "EDC", "%"]
	...
	for i in range (0, len(merchant)):
		...
		for j in range(0,len(merchant[i])):
			...
			if jumlahEDC:
				...
				sheet1.write(i+offRow+4*shiftDown, j+offCol, merchant[i][j], style)
	...
	for k in range (0, len(brilinks)):
		...
		for l in range(0,len(brilinks[k])):
			...
			if jumlahEDC:
				...
				style = xlwt.easyxf(contentStyle)
				sheet1.write(k+offRow+4*shiftDown, l+offCol+shiftLeft, brilinks[k][l], style)


	book.save(RegionName + "-EDC " +jenisPersen + "-" +time.strftime("%Y%m%d-%H")+'.xls')
