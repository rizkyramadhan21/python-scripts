...
def getAvailabilityRank(table):
	...
	try:
		...
		for i in range (2, numRows-1):
			...
			percentAvailMer = float(tdcells[9].getText())
			ukerName = cleanUpNamaUker(tdcells[0].getText())


			if (percentAvailMer >= 85.00):

				#arrBestBranch.append(ukerName+", "+jumlahATM)
				arrBestBranchMer.append(ukerName)
		
	except IndexError:

		arrBestBranchMer = getAvailabilityRank(table)

	return sorted(arrBestBranchMer)
...
if len(arrBestBranchMer):

		msgBody = msgBody + " > 85% AVAILABILITY: "+str(len(arrBestBranchMer)) + " KANCA \n\n* " + "\n* ".join(arrBestBranchMer)
AvailText = msgBody



msgBody = "----------------------------------------------------\n"
msgBody = msgBody + "KANCA with BEST AVAILABILITY EDC MERCHANT\nKANWIL "+RegionName + "\n"
