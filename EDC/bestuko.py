...
##09.09.2015 16:08 bestedcuko,brilink,merchant
#=================================================================================#
...
def getAvailabilityRank(table):
	...
	try:
		...
		for i in range (2, numRows-1):
			...
			percentAvailUko = tdcells[25].getText()

			ukerName = cleanUpNamaUker(tdcells[0].getText())
			#jumlahATM = tdcells[3].getText()

			if (percentAvailUko == '100.00') and ("KANINS" not in ukerName.upper()):

				#arrBestBranch.append(ukerName+", "+jumlahATM)
				arrBestBranchUko.append(ukerName)

	except IndexError:

		arrBestBranchUko = getAvailabilityRank(table)

	return sorted(arrBestBranchUko)
...
def colorPercent(percentAvailUko):

	strColor = str(percentAvailUko)

	if percentAvailUko >= 0.00:
		strColor = "Merah"
	if percentAvailUko >= 87.00:
		strColor = "Kuning"
	if percentAvailUko >= 93.00:
		strColor = "Hijau Muda"
	if percentAvailUko >= 97.00:
		strColor = "Hijau Tua"
...
arrBestBranchUko = getAvailabilityRank(table)
#print arrBestBranchUko


msgBody =""

if len(arrBestBranchUko):

		msgBody = msgBody + "100% AVAILABILITY: "+str(len(arrBestBranchUko)) + " KANCA \n\n* " + "\n* ".join(arrBestBranchUko)
AvailTextuko = msgBody



msgBody = "----------------------------------------------------\n"
msgBody = msgBody + "KANCA with BEST AVAILABILITY EDC UKO\nKANWIL "+RegionName + "\n"
msgBody = msgBody + "per "+ time.strftime("%d-%m-%Y pukul %H:%M") + "\n"
msgBody = msgBody + "----------------------------------------------------\n"
msgBody = msgBody + AvailTextuko + "\n"
msgBody = msgBody + "----------------------------------------------------\n"
