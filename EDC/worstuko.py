...
##09.09.2015 16:08 worstedcuko,brilink,merchant
#=================================================================================#
...
def getLowestAvailability(table, percentThreshold):
	...
	try:
		...
		for i in range (2, numRows-1):
			...
			percentAvail = tdcells[25].getText()
			ukerName = cleanUpNamaUker(str(tdcells[0].getText()))

			#jumlahATM = tdcells[3].getText()

			if (float(percentAvail)<float(percentThreshold)) and ("KANINS" not in ukerName.upper()) and ("PT" not in ukerName.upper()) and ("KANWIL" not in ukerName.upper()) and ("OTHERS" not in ukerName.upper()):

				#arrSelectedBranch.append(ukerName+", "+jumlahATM)
				arrSelectedBranch.append(percentAvail +" "+ ukerName)

	except IndexError:

		arrSelectedBranch = getLowestAvailability(table)

	return sorted(arrSelectedBranch)
...
def worstMessage(percentThreshold):
	...
	arrSelectedBranch = getLowestAvailability(table, percentThreshold)

	msgBody =""

	if len(arrSelectedBranch):

			msgBody = msgBody + "LOWEST AVAILABILITY: "+str(len(arrSelectedBranch)) + " KANCA \n\n* " + "\n* ".join(arrSelectedBranch)
	...
	msgBody = "---------------------------------------\n"
	msgBody = msgBody + "KANCA with LOWEST AVAILABILITY EDC UKO\nKANWIL "+RegionName + "\n"
	msgBody = msgBody + "per "+ time.strftime("%d-%m-%Y pukul %H:%M") + "\n"
	msgBody = msgBody + "under "+ str(percentThreshold) + "% availability\n"
	msgBody = msgBody + "---------------------------------------\n"
...
if len(sys.argv) > 1:
	...
	percentThreshold = float(sys.argv[1])

	msgBody = worstMessage(percentThreshold)

	print msgBody

else:

	percentThreshold = 80.00
