...
################################################################################################
firstURL='http://172.18.65.56/lasmobilemikro/index.php/dashboard/load_content'
...
def NotifikasiLAS():
	...
	for i in range(0, 37):
		...
		msgBody = "NOTIFIKASI LAS MIKRO " + arrBranchName[i] +" (" +arrBranchCode[i] + ")\nper " + time.strftime("%d.%m.%Y-%H:%M") +"\n------------------------------------\n"
		...
		fName = scriptDirectory +"conf/LASMikroRecipients.888"
