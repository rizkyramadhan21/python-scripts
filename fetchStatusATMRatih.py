#!/usr/bin/python
#---------------------------------------
# fetchStatusATMRatih.py
# (c) Jansen A. Simanullang
# 06 Januari 2017 18:13:01
# to be used with telegram-bot plugin
#---------------------------------------
# usage: fetchStatusATMRatih [oos/off/ccr/co/cl] kode cabang
# script name followed by [oos/off/ccr/co/cl] and branch code
#---------------------------------------
...
def getTProRatih(table, strCmd):
	...
	for i in range (numRowsHead, numRows):
		...
		if tdcells:
			...
			if tdcells[colCMD[strCmd.upper()]].getText() == "FAIL":
				...
				TProRatih.append((strKodeCabang, namaCROUKO, intCRO, strReplenish, strLocation, strTID, strNamaCabang, strLastTunai))
			
	TProRatih = sorted(TProRatih, key=itemgetter(1, 3, 2), reverse = False)
...
if len(sys.argv) > 0:
	...
	try:
		...
		if strCmd.upper() not in ["OOS","OFF","CCR","CO","CL"]:
			print "tidak dikenal"			
		else:
			table=getLargestTable(getTableList(fetchHTML(alamatURL)))
			TProRatih = getTProRatih(table, strCmd)
			#print TProRatih
			#print strCmd+":", len(TProRatih)
			msgBody = getTOfflineCabang(TProRatih, strCmd.upper(), branchCode)
			print msgBody

	except:
		print "ANDA belum meng-input kode cabang.\nPilihan perintah:\n'OOS'\n'OFF'\n'CCR'\n'CO'\n'CL'"
