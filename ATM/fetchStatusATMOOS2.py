...
def getATMStats(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM OOS di wilayah kerja Anda."
	return msgBody
...
def getATMStatsCRO(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM OOS CRO kategori ini di wilayah kerja Anda."
	return msgBody
...
def getATMStatsUKO(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM OOS UKO kategori ini di wilayah kerja Anda."
	return msgBody
...
def getATMStatsIDM(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM OOS INDOMARET kategori ini di wilayah kerja Anda."
...
def getATMStatsALFA(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM OOS ALFA kategori ini di wilayah kerja Anda."
...
if len(sys.argv) > 0:
	...
	if AREAID.isdigit():
		...
		alamatURL = "http://172.18.65.42/statusatm/viewbyregionprobcabang.pl?AREAID="+ AREAID + "&ERROR=OOS"

		msgBody = getATMStats(table=getWidestTable(getTableList(fetchHTML(alamatURL))))

		if msgBody:	

			msgBody = strHeaderLine +"ATM OOS "+ AREAID.upper() +timestamp+ strHeaderLine + msgBody
			print msgBody
