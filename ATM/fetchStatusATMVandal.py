	...
	for i in range (0, numRows):
		...
		if tdcells:
			...
			if "ATM CENTER" in tdcells[6].getText():
				...
				## kode filter Vandal kolom 10 indeks 9
				if "VANDAL" in tdcells[9].getText().upper():
...
if len(sys.argv) > 0:
	...
	if AREAID.lower() == "cro":
		...
		if msgBody:	
			...
			msgBody = strHeaderLine +"ATM PROBLEM VANDAL "+ AREAID.upper() +timestamp+ strHeaderLine + msgBody
			print msgBody
