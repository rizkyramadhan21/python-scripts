...
def getATMStats(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM NOP UKO di wilayah kerja Anda."
...
def getATMStatsCRO(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM NOP CRO kategori ini di wilayah kerja Anda."
...
def getATMStatsUKO(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM NOP UKO kategori ini di wilayah kerja Anda."
...
def getATMStatsIDM(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM NOP INDOMARET kategori ini di wilayah kerja Anda."
...
def getATMStatsALFA(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM NOP ALFA kategori ini di wilayah kerja Anda."
...
if len(sys.argv) > 0:
	...
	if AREAID.isdigit():
		...
		alamatURL = "http://172.18.65.42/statusatm/viewbynopcabang.pl?AREAID="+ AREAID + "&gr=Y"

		msgBody = getATMStats(table=getWidestTable(getTableList(fetchHTML(alamatURL))))

		if msgBody:	

			msgBody = strHeaderLine +"ATM NOP GARANSI "+ AREAID + timestamp + strHeaderLine + msgBody
			print msgBody

		alamatURL = "http://172.18.65.42/statusatm/viewbynopcabang.pl?AREAID="+ AREAID + "&gr=N"

		msgBody = getATMStats(table=getWidestTable(getTableList(fetchHTML(alamatURL))))

		if msgBody:	

			msgBody = strHeaderLine +"ATM NOP NON GARANSI "+ AREAID + timestamp + strHeaderLine + msgBody
			print msgBody
	...
	if AREAID.lower() == "cro":
		...
		if msgBody:	
			...
			msgBody = strHeaderLine +"ATM NOP GARANSI "+ AREAID.upper() + timestamp + strHeaderLine + msgBody
			print msgBody
		...
		if msgBody:	
			...
			msgBody = strHeaderLine +"ATM NOP NON GARANSI "+ AREAID.upper() + timestamp + strHeaderLine + msgBody
			print msgBody
