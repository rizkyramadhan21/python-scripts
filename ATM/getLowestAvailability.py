...
def getLowestAvailability(table, percentThreshold):
	...
	try:
		...
		for i in range (numRowsHead, numRows):
			...
			if (float(percentAvail)<float(percentThreshold)) and (ukerName != "KANWIL JAKARTA 3") and ("KCP" not in ukerName and ukerName):
...
def worstMessage(percentThreshold):
	...
	msgBody = "---------------------------------------\n"
	msgBody = msgBody + "KANCA with LOWEST AVAILABILITY ATM\nKANWIL "+RegionName + "\n"
