...
def getATMStatsCRO(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM EPP CRO kategori ini di wilayah kerja Anda."
	return msgBody
...
def getATMStatsUKO(table):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM EPP UKO kategori ini di wilayah kerja Anda."
	return msgBody
...
if len(sys.argv) > 0:
	...
	if AREAID.isdigit():
		...
		if msgBody:	
			...
			msgBody = "cara penggunaan:\n!status epp [cro|uko]\nmengambil status ATM yang bermasalah pada EPP"
			print msgBody


	if AREAID.lower() == "cro":

		alamatURL = "http://172.18.65.42/statusatm/viewbyregionprobooscr.pl?REGID=15&ERROR=EPP_ST&gr=Y"

		msgBody = getATMStatsCRO(table=getWidestTable(getTableList(fetchHTML(alamatURL))))

		if msgBody:	

			msgBody = strHeaderLine +"ATM EPP GARANSI "+ AREAID.upper() +timestamp+ strHeaderLine + msgBody
			print msgBody

		alamatURL = "http://172.18.65.42/statusatm/viewbyregionprobooscr.pl?REGID=15&ERROR=EPP_ST&gr=N"

		msgBody = getATMStatsCRO(table=getWidestTable(getTableList(fetchHTML(alamatURL))))

		if msgBody:	

			msgBody = strHeaderLine +"ATM EPP NON GARANSI "+ AREAID.upper() +timestamp+ strHeaderLine + msgBody
			print msgBody
