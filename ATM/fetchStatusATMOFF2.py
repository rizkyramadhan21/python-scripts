...
#---------------------------------------
# usage: fetchStatusATMOFF2 cro/uko/kode cabang
...
def getTOfflineCabang(TOffline, branchCode):
	...
	if TOfflineUKO or TOfflineCRO:
		msgBody = strHeaderLine +"ATM OFFLINE (>6 JAM) "+ strNamaCabang.upper() +timestamp+ strHeaderLine + msgBody + "\n"
	else:

		msgBody = strHeaderLine +"ATM OFFLINE (>6 JAM) "+ branchCode +timestamp+ strHeaderLine + msgBody + "\nTidak ada ATM kategori ini di wilayah Anda!"
...
def getTOfflineCRO(TOffline, selectedCRO):
	...
	if TOfflineCRO:
		...
		msgBody += "\n"+regionName + "-[TOTAL OFFLINE CRO (> 6 JAM) "+selectedCRO+"]: "+str(counter)
...
def getTOfflineUKO(TOffline):
	...
	if TOfflineUKO:
		...
		msgBody += "\n"+regionName + "-[TOTAL OFFLINE UKO (> 6 JAM)]: "+str(counter)
...
def getTOfflineACI(TOffline, selectedACI):
	...
	if TOfflineACI:
		...
		msgBody += "\n"+regionName + "-[TOTAL OFFLINE "+selectedACI.upper()+" (> 6 JAM)]: "+str(counter)
		...
		if AREAID[0].isalpha():
			...
			if AREAID.upper() == "UKO":
				...
				try:
					...
					if msgBody:	
						msgBody = strHeaderLine +"ATM OFFLINE (> 6 JAM) "+ AREAID.upper() + " - "+regionName+ timestamp+ strHeaderLine + msgBody
						print msgBody
