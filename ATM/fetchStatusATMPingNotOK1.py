#!/usr/bin/python
#---------------------------------------
# fetchStatusATMPingNotOK.py
# (c) Jansen A. Simanullang, 11:15:55
# 30.12.2015 19:23:17
# 05.01.2016
# to be used with telegram-bot plugin
#---------------------------------------
# usage: fetchStatusATMPingNotOK cro/uko/ kode cabang
	...
	for i in range (0, numRows):
		...
		if tdcells:
			...
			if "ATM CENTER" in tdcells[7].getText():
				...
				## kode filter ping not ok kolom 10 indeks 9
				if "PING NOT OK" in tdcells[9].getText():
	...
	for i in range (0, numRows):
		...
		if tdcells:
			...
			if "PING NOT OK" in tdcells[9].getText():
				...
				## kode filter ping not ok kolom 10 indeks 9
				if branchCode in tdcells[6].getText():
...
if len(sys.argv) > 0:
	...
	if msgBody:	
		...
		msgBody = strHeaderLine +"ATM PING NOT OK - OFFLINE "+ AREAID.upper() + timestamp + strHeaderLine + msgBody
		print msgBody
