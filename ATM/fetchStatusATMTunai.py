#!/usr/bin/python
#---------------------------------------
# fetchStatusATMTunai.py
# (c) Jansen A. Simanullang, 11:15:55
# 14 Januari 2106 09:04:11
# to be used with telegram-bot plugin
#---------------------------------------
# usage: fetchStatusATMTunai cro/uko/kode cabang
...
def getATMStats(table, branchCode):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM TUNAI UKO kategori ini di wilayah kerja Anda."
	return msgBody



def getATMStatsCRO(table, branchCode):
	...
	if msgBody == "":
		msgBody = "Tidak ada ATM TUNAI CRO kategori ini di wilayah kerja Anda."
	return msgBody


def getATMStatsUKO(table, branchCode):
...
if len(sys.argv) > 0:
	...
	if AREAID.isdigit():
		...
		if msgBody:	
			...
			msgBody = strHeaderLine +"ATM TUNAI "+ AREAID.upper() +timestamp+ strHeaderLine + msgBody
			print msgBody
	...
	if AREAID.lower() == "cro":
		...
		try:
			...
			msgBody = getATMStatsCRO(table=getWidestTable(getTableList(fetchHTML(alamatURL))), branchCode = sys.argv[2])

			if msgBody:	

				msgBody = strHeaderLine +"ATM TUNAI "+ AREAID.upper() + " - "+ branchCode +timestamp+ strHeaderLine + msgBody
				print msgBody

		except:

			msgBody = "Anda belum menyebutkan kode cabang! (Branch code not specified!) \n\nusage:\n!tunai cro [kode cabang]"
			print msgBody
	...
	if AREAID.lower() == "uko":
		...
		try:
			...
			msgBody = getATMStatsUKO(table=getWidestTable(getTableList(fetchHTML(alamatURL))), branchCode = sys.argv[2])
